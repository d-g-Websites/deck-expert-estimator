...
app.get("/api/pricing/deck", requireAuth, (req, res) => {
  res.json({
    ...
    two_color_surcharge: TWO_COLOR_SURCHARGE,
  ...
  res.json({
    ...
    project: (() => {
      ...
      return {
        ...
        repairs_description: row.repairs_description,
        warranty_waived: !!row.warranty_waived,
        stain_underside: !!row.stain_underside,
      };
    })(),
    labels: {
...
app.post("/api/estimate", requireAuth, estimateUpload.fields([
  ...
  try {
    ...
    const stain_vertical = b.stain_vertical ? 1 : 0;
    const stain_vertical_sqft = b.stain_vertical_sqft ? parseFloat(b.stain_vertical_sqft) : 0;
    const stain_two_color = b.stain_two_color ? 1 : 0;
    const stain_two_color_mode = (b.stain_two_color_mode && TWO_COLOR_SURCHARGE[b.stain_two_color_mode]) ? b.stain_two_color_mode : null;
    ...
    const repairs_description = (b.repairs_description || "").toString().slice(0, 8000).trim() || null;
    const warranty_waived = b.warranty_waived ? 1 : 0;
    const stain_underside = b.stain_underside ? 1 : 0;
    ...
    const breakdown = computeDeckEstimate({
      wood_type, deck_location, cleaning_enabled, surface_sqft, has_railing, has_metal_spindles, railing_lf, multilevel_levels,
      light_clean, chicago_surcharge, pergola_gazebo_size,
      sanding_condition, vertical_sanding, vertical_length, vertical_height,
      staining_enabled, stain_process, stain_customer_supplied, stain_vertical, stain_vertical_sqft,
      stain_two_color, stain_two_color_mode, stain_underside,
